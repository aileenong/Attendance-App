...
def upload_samples_ui():
    ...
    files = st.file_uploader("Upload multiple images", type=["png", "jpg", "jpeg"], accept_multiple_files=True, key=f"uploader_{empid}")
...
def delete_corrupted_files(corrupted_files):   # new function - delete corrupted or empty files
    ...
    for path in corrupted_files:
        try:
            supabase.storage.from_(FACES_BUCKET).remove([path])
            st.write(f"Deleted corrupted file: {path}")
        except Exception as e:
...
def retrain_lbph_from_storage_ui4():  # to  be removed if the updated one works
    ...
    if st.button("Retrain now"):
        try:
            ...
            if corrupted_files:
                st.warning("The following files were empty or corrupted and skipped:")
                for cf in corrupted_files:
                    st.write(f"- {cf}")

                if st.button("Delete corrupted files from Supabase"):
...
def main():
    ...
    user = st.session_state.get("user")
    role = user.get("role", "attendance_only")  # default role if missing
    ...
    if role == "attendance_only":
        page = st.sidebar.radio(
            ...
            index=0     # default to "Mark attendance"
        )
    else:
        page = st.sidebar.radio(
            "Menu",
            ...
            index=6     # default to "Mark attendance"
        )
